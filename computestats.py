...
def f(csdij,psdi,psdj,f,df,flim):
    ...
    stats = {'meanphase':np.zeros((fbin.shape)),'stdphase':np.zeros((fbin.shape)),
             'meanpow':np.zeros((fbin.shape)),'stdpow':np.zeros((fbin.shape)),
             'coherence':np.zeros((fbin.shape),dtype=complex),'fRFTbin':fbin}
